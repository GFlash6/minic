...
## Copilot (GitHub Copilot Chat) 支持

本项目包含一个针对 GitHub Copilot Chat 的会话监视器 `scripts/copilot_session_watch.py`，用于监视 VS Code 的 Copilot Chat JSONL 会话文件（位于 `%%APPDATA%%/Code/User/workspaceStorage` 下），并通过现有的 Hub/transport 将映射好的动画发送到设备。

要手动运行 Copilot 监视器：

```powershell
python skills/codex-clawd-status/scripts/copilot_session_watch.py --follow-latest
```

可通过同样的后台 UI 控制器或 Windows 启动快捷方式来启动该监视器（参见下文 “Daily Start” 中的 `Clawd Hub App.lnk`）。监视器会使用 `codex_clawd_hook.py` 中的交付逻辑，因此不需要额外的 hook 脚本。

覆盖默认 Watcher 客户端 ID：

```powershell
$env:CLAWD_TANK_WATCH_CLIENT_ID = "my-copilot-watch"
```
