...
The app keeps the local Hub and Copilot session watcher alive, shows module
...
COPILOT_WATCHER_SCRIPT = SCRIPT_DIR / "copilot_session_watch.py"
CODEX_WATCHER_SCRIPT = SCRIPT_DIR / "codex_session_watch.py"
    ...
    def restart_watcher(self) -> dict[str, Any]:
        try:
            return http_json(
                self.hub_url + "/module/restart",
                method="POST",
                body={"module": "copilot-watcher"},
                timeout=3.0,
            )
        except Exception as exc:
            watcher = COPILOT_WATCHER_SCRIPT if COPILOT_WATCHER_SCRIPT.exists() else CODEX_WATCHER_SCRIPT
            if not watcher.exists():
                return {"ok": False, "error": "copilot_session_watch.py not found"}
            subprocess.Popen([sys.executable, str(watcher), "--follow-latest"], **process_kwargs())
            return {"ok": True, "message": f"watcher start requested after {exc}"}

    def ensure_watcher(self) -> None:
        try:
            modules = self.modules()
            watcher = modules.get("copilot-watcher", {})
